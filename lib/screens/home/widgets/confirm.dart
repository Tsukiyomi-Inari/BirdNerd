...
late  bool _loading = false;
  ...
  Future<String> cropCircle() async {
    ...
    img.Image cropImage =
        img.decodeImage(File(globals.filepath).readAsBytesSync())!;
    // 2) Calculate the diameter of the circle as a percentage of the smaller dimension
    int diameter = (cropImage.width < cropImage.height
        ? cropImage.width
        : cropImage.height);
    ...
    img.Image croppedImage = img.copyCrop(cropImage,
        x: x, y: y, width: diameter, height: diameter, radius: diameter / 2);
    var rgba = croppedImage.convert(
        numChannels: 4); // make sure the image has an alpha channel
    ...
    final tempFile = File('${tempDir.path}-$date-$uid.png'); // create temp file
    tempFile.writeAsBytesSync(
        img.encodePng(croppedImage)); // write image bytes to file
  ...
  Future<void> generateMapMarker(Bird bird, uid) async {
    ...
    /// Retrieve bird name
    DatabaseService.updateUserMarkers(uid!, bird.commonName, formattedDate,
        globals.latitude!, globals.longitude!, markerIcon);
  }
                      ...
                      child: Column(
                          ...
                          children: [
                            ...
                            Padding(
                              ...
                              child: Text(
                                _recognized
                                    ? 'Confirm the match and view your discovery on the life list page!'
